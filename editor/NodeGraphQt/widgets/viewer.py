...
from editor.NodeGraphQt.base.menu import BaseMenu
from editor.NodeGraphQt.constants import (
    LayoutDirectionEnum,
    PortTypeEnum,
    PipeEnum,
    PipeLayoutEnum,
...
from editor.NodeGraphQt.qgraphics.node_abstract import AbstractNodeItem
from editor.NodeGraphQt.qgraphics.node_backdrop import BackdropNodeItem
from editor.NodeGraphQt.qgraphics.pipe import PipeItem, LivePipeItem
from editor.NodeGraphQt.qgraphics.port import PortItem
from editor.NodeGraphQt.qgraphics.slicer import SlicerPipeItem
from editor.NodeGraphQt.widgets.dialogs import BaseDialog, FileDialog
from editor.NodeGraphQt.widgets.scene import NodeScene
from editor.NodeGraphQt.widgets.tab_search import TabSearchMenuWidget
...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    search_triggered = Signal(str, tuple)
    connection_sliced = Signal(list)
    connection_changed = Signal(list, list)
    insert_node = Signal(object, str, object)
    node_name_changed = Signal(str, str)
    node_backdrop_updated = Signal(str, str, object)

    # pass through signals that are translated into "NodeGraph()" signals.
    node_selected = Signal(str)
    node_selection_changed = Signal(list, list)
    node_double_clicked = Signal(str)
    data_dropped = Signal(QtCore.QMimeData, object)
    context_menu_prompt = Signal(str, object)

    def __init__(self, parent=None, undo_stack=None):
        """
        Args:
            parent:
            undo_stack (QUndoStack): undo stack from the parent
                                               graph controller.
        """
