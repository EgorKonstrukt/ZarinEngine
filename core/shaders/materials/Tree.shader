...
uniform vec4 u_cascade_rects[4];
            ...
            float compute_shadow() {
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int j = -1; j <= 1; j++) {
                        vec4 crect = u_cascade_rects[cascade_idx];
                        vec2 texel_size = vec2(1.0) / vec2(textureSize(u_cascade_atlas, 0));
                        vec2 o = vec2(float(i), float(j));
                        vec2 ro = vec2(o.x * ca - o.y * sa, o.x * sa + o.y * ca);
                        vec2 uv = crect.xy + clamp(proj_coords.xy + ro * texel_size, vec2(0.001), vec2(0.999)) * crect.zw;
                        float pcf_depth = texture(u_cascade_atlas, uv).r;
                        float w = 1.0;
                        if (i == 0) w += 1.0;
                        if (j == 0) w += 1.0;
